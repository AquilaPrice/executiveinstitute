...
const PageHero = ({
  ...
  return (
    <section className="relative w-full overflow-hidden" style={{ minHeight: "110vh" }}>
      ...
      <div className="relative z-10 flex min-h-[110vh] flex-col items-center justify-center text-center text-white container-tight py-28">
        {/* Badge */}
        ...
        {/* Serif headline */}
        <h1
          className="heading-serif mt-8 text-5xl sm:text-6xl md:text-7xl lg:text-[5.5rem] text-balance max-w-5xl animate-fade-in"
        ...
        <form
          onSubmit={onSubmit}
          className="mt-12 w-full max-w-2xl animate-slide-up"
          style={{ animationDelay: "450ms" }}
        >
          <div className="glow-multi flex items-center gap-2 rounded-2xl bg-white p-2 pl-5">
            ...
            <input
              ...
              className="flex-1 bg-transparent border-0 outline-none text-primary placeholder:text-primary/40 text-base py-3"
              aria-label="Search programs"
            />
            <button
              type="submit"
              className="inline-flex items-center gap-2 rounded-xl bg-gradient-accent text-accent-foreground font-semibold px-5 py-3 shadow-glow hover:scale-[1.03] transition-transform"
            >
              Search <ArrowRight className="h-4 w-4" />
            </button>
          </div>
          <p className="mt-3 text-xs text-white/60 uppercase tracking-wider">
